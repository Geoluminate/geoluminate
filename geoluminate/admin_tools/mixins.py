...
class DownloadMixin:
    ...
    def download(self, request, *args, **kwargs):
        ...
        for key, qs in self.get_object().get_data().items():
            ...
            references = references | Publication.objects.filter(
                sites__in=sites).distinct()

        # write references to .bib file
        if references:
            zf.writestr(
                f'{self.get_object()}.bib',
                self.references_to_bibtex(references))

        return response

    def references_to_bibtex(self, references):
        # add bibtex file to zip object
        references = references.distinct().exclude(
            bibtex__isnull=True).values_list(
            'bibtex', flat=True)
        buffer = StringIO()
        buffer.write('\n\n'.join(references))
